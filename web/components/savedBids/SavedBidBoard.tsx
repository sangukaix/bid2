...
import ProposalGeneratorButton from "@/components/proposal/ProposalGeneratorButton";
  ...
  return (
    <section className="mt-6 overflow-hidden rounded-lg border border-slate-200 bg-white">
      <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
        <h2 className="text-base font-semibold text-slate-600">내 저장 공고</h2>
                ...
                <tr className="hover:bg-slate-50" key={`${bid.bidNtceNo}-${bid.bidNtceOrd}`}>
                  ...
                  <td className="px-3 py-4">
                    <div className="flex items-center justify-center gap-1.5 whitespace-nowrap">
                      ...
                      <Link className="inline-flex h-10 w-[76px] cursor-pointer flex-col items-center justify-center rounded-md border border-blue-200 bg-blue-50 text-xs font-semibold leading-4 text-blue-700 transition hover:bg-blue-100" href={`/dashBoard/matchBid?bid=${encodeURIComponent(bid.bidNtceNo)}`}>
                        <span>AI 분석</span>
                        {bid.hasAnalysis && <span className="text-[10px] font-normal">다시보기</span>}
                      </Link>
                      <ProposalGeneratorButton
                        bidNtceNo={bid.bidNtceNo}
                        bidTitle={bid.bidNtceNm}
                        initialHasProposal={bid.hasProposal}
                      />
